...
export function draftResponse(dispute, verification = verifyEvidence(dispute)) {
  ...
  const draftText = [
    ...
    `Our review indicates a ${Math.round(scoring.winProbability * 100)}% contest success likelihood based on merchant-side fulfillment and customer evidence.`,
    ...
    `Recommended action: ${scoring.decision.replaceAll("_", " ")}.`
  ].join(" ");
